...
gotg = GotGraph(auth=('neo4j', 'dbuserdbuser'), host="localhost", port=7687, secure=False, encrypted=False)


def get_characters_by_query(args):
    res = rdb.get_by_by_query("gotf20raw", "characters", args)
    return res
...
def get_character_by_id(ch_id):
    ...
    res = rdb.get_by_by_query("gotf20raw", "characters", q)
